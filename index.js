...
function makeSmartPerson(name) {
  ...
  let person = new Object();
  person.name = name;
  // person.sum = function(num1, num2){return num1 + num2;};
  // person.speak = function(){return "Hello, my name is ${name}";};
  person.sum = (num1, num2) =>  num1 + num2;
  person.speak = () => `Hello, my name is ${name}`;
  return person;
}
...
function getCarInfoByIndex(inventory, indexNum) {
  /* code here */
  const car = inventory.find((item, index) => { return index === indexNum}) ;
...
*/
  function getLastCarInfo(inventory) {
    /* code here */
    // let car = inventory.pop();
    let indexID = inventory.length - 1;
    // const car = inventory.find((item, index) => {return index = indexID});
    const car = inventory[indexID];
    return `This is a ${car.car_make} ${car.car_model}`;
  }
...
function getCarInfoById(inventory, id) {
  /* code here */
  const car = inventory.find((item, index) => {return item.id === id;});
  return `This is a ${car.car_make} ${car.car_model}`;
}
...
function sortCarInventory(inventory) {
  /* code here */
  function compare1(carA,carB){
    let modelA = carA.car_model.toUpperCase();
    let modelB = carB.car_model.toUpperCase();
    let comparison = 0;
    if (modelA > modelB){ comparison = 1;}
    else if (modelA < modelB){comparison = -1;}
    return comparison;
  }
  return inventory.sort(compare1);
}
...
function getGermanCars(inventory) {
  let german_makes = ["Audi", "Mercedes-Benz", "Volkswagen", "BMW"];
  let germanCars = [];
  for (let i=0; i<inventory.length; i++){
    if (german_makes.includes(inventory[i].car_make)){
...
function carMaker(odometerReading) {
  let odometerObj = new Object();
  odometerObj.odometer = odometerReading;
  odometerObj.drive = function(distance){ 
